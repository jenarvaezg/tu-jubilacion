...
import { buildChartData, nominalKey, realKey } from "../../src/hooks/use-chart-data";
...
describe("buildChartData", () => {
  it("covers full age range from current age to 90", () => {
    const profile = DEFAULT_STATE.calculation.profile;
    ...
    expect(chartData.length).toBe(90 - profile.age + 1);
    expect(chartData[0].age).toBe(profile.age);
  ...
  it("keeps pension values undefined before retirement and fills them at retirement", () => {
    ...
    const beforeRetirement = chartData.find((p) => p.age === profile.age);
