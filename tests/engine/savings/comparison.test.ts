...
describe("generateComparisonTimeline", () => {
  ...
  it("starts at 0 for all asset classes at current age", () => {
    const timeline = generateComparisonTimeline(params);
    const first = timeline[0];
    expect(first.equityPortfolioReal).toBe(0);
    expect(first.bondsPortfolioReal).toBe(0);
    expect(first.depositsPortfolioReal).toBe(0);
  });
  ...
  it("equity portfolio is significantly larger than deposits at 30+ years", () => {
    const timeline = generateComparisonTimeline(params);
    const last = timeline[timeline.length - 1];
    // With 32 years at 5% vs 0%, equity should be at least 50% more
    expect(last.equityPortfolioReal).toBeGreaterThan(
      last.depositsPortfolioReal * 1.5,
    );
  });

  it("deposits portfolio equals simple contribution sum (0% real return)", () => {
    const timeline = generateComparisonTimeline(params);
    const last = timeline[timeline.length - 1];
    const totalContributed = 200 * 32 * 12;
  ...
  it("handles 0 contribution", () => {
    ...
    const last = timeline[timeline.length - 1];
    expect(last.equityPortfolioReal).toBe(0);
  });
});
